...
public class Boss_GSP2 : MonoBehaviour
{
    #region Singleton
    public static Boss_GSP2 Ins;
    void Awake()
    {
        Ins = this;
    }
    ...
    public GameObject boss1Prefabs;
    ...
    public void SetBoss(bool isBossBlue)
    {
        if (isBossBlue)
        {
            anim = Instantiate(boss1Prefabs, transform.position, Quaternion.identity, transform).GetComponent<Animator>();
        }
        else
        {
            anim = Instantiate(boss2Prefabs, transform.position, Quaternion.identity, transform).GetComponent<Animator>();
        }
        anim.transform.localPosition = Vector3.up * (-0.5f);
        anim.transform.localScale = Vector3.one * 0.7f;
    }
    ...
    public void DestroyBoss()
    {
        ...
        EndCard_GSP1.Ins.WaitToShowEndCard(5f);
